...
object SettingsManager {
    ...
    private fun ensureAutoSubscription() {
        val autoUrl = com.v2ray.ang.BuildConfig.AUTO_SUB_URL
        if (autoUrl.isBlank()) return
        val existing = MmkvManager.decodeSubscriptions().firstOrNull { it.subscription.url == autoUrl }
        if (existing != null) return
        val subId = Utils.getUuid()
        val item = SubscriptionItem(
            remarks = "Auto Aggregated (PC)",
            url = autoUrl,
            enabled = true,
            autoUpdate = true,
            updateInterval = 360L,
